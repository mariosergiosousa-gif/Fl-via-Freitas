...
export default function TreatmentDetailsModal({ isOpen, onClose, title, content }: TreatmentDetailsModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <div key="treatment-modal-overlay" className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-10">
                <div className="max-w-[80%]">
                  ...
                  <h2 className="serif text-[28px] md:text-3xl text-white uppercase italic leading-tight">
                    {title}
                  </h2>
                </div>
                <button 
